...
//    RHC has no params. Just run it directly
        ...
        double best_temp = 1E12;
        double best_cooling = 0.95;
...
//    Convert nanoseconds to seconds
     ...
     */
    private static void runRHC(int trainIterations) {
...
//    Convert nanoseconds to seconds
    ...
    private static void determineSAParams(int trainIterations) {
...
//      Find the average error for this network configuration
    ...
    private static void runSA(int trainIterations, double temp, double cooling) {
...
//    Convert nanoseconds to seconds
    ...
    private static void determineGAParams(int trainIterations) {
...
//      Find the average error for this configuration
     ...
     */
    private static void runGA(int trainIterations) {
...
//    Convert nanoseconds to seconds
    ...
    private static double evaluateNetwork(BackPropagationNetwork network, Instance[] data) {
        ...
        for (Instance aData : data) {
            network.setInputValues(aData.getData());
            network.run();

            Vector actual = aData.getLabel().getData();
...
//    Where the actual max should be
    ...
    private static void initializeInstances() {
        ...
        Set<String> unique_labels = new HashSet<>();
...
//    Creating a mapping of bitvectors. So "some classification" => [0, 1, 0, 0]
        ...
        Map<String, double[]> bitvector_mappings = new HashMap<>();
...
//    This works even for binary classification
    ...
    private static void makeTestTrainSets() {
        ...
        Collections.addAll(instances, allInstances);
...
//    System.out.println("Train Set ")
    ...
    private static List<Instance[]> kfolds(DataSet training) {
        ...
        List<Instance> instances = new ArrayList<>();
        Collections.addAll(instances, trainInstances);
...
//    Number of values per fold
    ...
    private static Instance[] combineInstances(List<Instance[]> instanceList) {
        ...
        for (Instance[] fold: instanceList) {
            ...
            Collections.addAll(combined, fold);
        }
    ...
    private static Instance[] getTrainFolds(List<Instance[]> folds, int foldIndex) {
        ...
        return combineInstances(trainFolds);
    }
